...
| --- | --- | --- |
| v1.0 | - | 初稿 |
| v1.1 | 2026-08-27 | 重构文档结构；修复条目编号错误；补充固定格式规范（.gg 文件、资产、导出格式等） |
| v1.2 | 2026-08-27 | 补充 .gg 文件 `data` 字段详细结构；新增技术选型、一键生成 .exe 实现方案、目录结构、开发路线图 |
| v1.3 | 2026-08-27 | 技术栈改为 pywebview（管理器端与游戏端统一为 Web 前端 + Python 后端）；更新目录结构 |
| v1.4 | 2026-08-27 | 完成阶段 3：游戏端基础阅读系统与分支选项系统 |
| v1.5 | 2026-08-27 | 完成阶段 4：存档/读档、标题主菜单、画廊、音乐室、剧本回想、设置、结局统计；接入 BGM/语音播放 |
| v1.6 | 2026-08-27 | 完成阶段 5：一键生成 `.exe`（校验→组装→PyInstaller 单文件/目录，后台线程 + 进度日志） |
| v1.7 | 2026-08-27 | 构建产物去除控制台窗口（`--noconsole`）；生成页新增进度条；游戏端启动自动全屏、最小窗口 800×600；提供桌面示例项目 |
| v1.7.1 | 2026-08-27 | 修复打包后 `runtime` 模块缺失；游戏端剧情结束时按章节分支条件判定结局；解锁的隐藏剧情可在鉴赏模式播放；修正示例项目分支串线 |
| v1.7.2 | 2026-08-27 | 支持多章节自动推进（上一章剧情结束后进入下一章）；修复存档被误拦（结局界面可存档）；示例扩展为三章多对话 |
| v1.7.3 | 2026-08-27 | 支持选项**跨脚本跳转**（jump_to 可指向另一脚本 ID），选项真正产生分支路径；示例重构为 8 个脚本的分叉剧情 |
| v1.7.4 | 2026-08-27 | 选项编辑器跳转目标列表加入其他剧情（跨脚本）；解锁的隐藏剧情在未显式跳转时自动播放；示例「去天台吹风」跳转到 script_hidden |
| v1.7.5 | 2026-08-27 | 示例第二章加入好感度门槛分支（`affection < 15` → 普通结局），低好感路线不再进入第三章告白 |
...
- 添加角色简介、性格说明等文本信息；
- 进入"角色资产"界面管理角色的图片、音频等信息，支持上传文件、命名文件、查看文件详细信息等操作。
...
编辑单个章节的剧情，使用角色选择器、场景选择器、文本输入框等控件获取信息，存储每一条对话数据。

每条对话包含以下信息：

| 数据项 | 说明 |
| --- | --- |
| 角色 | 角色 ID、名称、立绘编号、表情、播放对话音频等 |
| 场景 | 场景 ID、名称等 |
| 背景音乐 | 当前对话使用的 BGM |
| 对话内容 | 实际展示的文本 |
...
- **文本（Text）**：供玩家直接阅读；
- **选项（Choice）**：需要玩家进行选择，每个选项可"绑定"到不同结果，例如跳转至某个对话、播放特定 CG、角色好感度增减、解锁隐藏剧情、指向某一结局等。

支持的编辑操作：

- 对话列表支持插入、排序、编辑、删除、复制等操作；
...
- **资产分类**：图片（背景图、场景图、角色立绘、CG 事件图、UI 资产）、视频、音频（BGM、音效、角色语音等）；
- **筛选模式**：按角色、按场景等；
- **排序模式**：按上传日期、按引用次数等；
...
设置项目信息，如项目名称、作者、版本号等。

### 2.9 AI 功能（规划中）

后续计划在 GalGen 管理器中接入 AI 功能，新增页面用以配置 AI 模型 API 等信息，用于一键生成角色音频、游戏内置自由对话系统等。
...
`scripts` 为对象数组，每个剧情包含一组按顺序播放的对话，结构如下：
      ...
      "options": [
        {
          ...
          "jump_to": "dlg_0010",
          "effects": [
            { "target": "char_0001", "variable": "affection", "operation": "add", "value": 5 }
          ],
          "unlock_cg": "asset_0203",
          "unlock_script": "script_0005",
          "ending_id": null
        },
        {
          "id": "opt_0002",
...
| `content` | string | 对话内容 |
| `options` | array | 选项列表（`choice` 类型必填） |
...
| `id` | string | 选项 ID |
| `content` | string | 选项文本 |
| `jump_to` | string | 跳转目标：同剧情内的对话 ID，**或另一剧情的脚本 ID（跨脚本分支）**；为空表示剧情结束 |
| `effects` | array | 变量修改效果列表 |
| `unlock_cg` | string | 解锁的 CG 资产 ID（可为空） |
| `unlock_script` | string | 解锁的隐藏剧情 ID（可为空） |
| `ending_id` | string | 指向的结局 ID（可为空） |
...
| `category` | enum | `bg` / `scene` / `standee` / `cg` / `ui` / `bgm` / `se` / `voice` / `video` |
...
- 项目、角色、场景、章节的 ID 建议采用「小写前缀 + 递增序号」的格式（如 `char_0001`），保持唯一性与可读性；
- 项目版本号遵循语义化版本号（SemVer）规范，即 `主版本号.次版本号.修订号`；
...
5. **构建**：管理器自动生成 PyInstaller `.spec`（含图标、版本信息、可选的 UPX 压缩），调用打包命令生成单文件或目录形态的 `.exe`。
6. **产物输出**：生成 `<游戏名>_v<版本号>.exe` 并放置于项目 `dist/` 目录；支持一键打开所在目录。
...
galgen/
├── project.md               # 本文档
├── requirements.txt         # 依赖（pywebview、pyinstaller）
├── manager/                 # 管理器端
│   ├── main.py              # pywebview 入口
│   ├── web/                 # 前端静态资源与后端桥接
│   │   ├── index.html       # 单页应用骨架
│   │   ├── css/             # 主题样式
│   │   ├── js/              # app.js（核心）/ pages.js（页面）/ plots.js（剧情编辑器）
│   │   └── api.py           # JS API 桥接层（项目/CRUD/资产/校验/导出）
│   ├── core/                # 核心逻辑（.gg 读写、校验、导出、配置、资产文件）
│   └── builder/             # 生成 / 打包流程
...
│   │   ├── index.html       # 游戏主界面
│   │   ├── css/             # 游戏主题样式
│   │   └── js/game.js       # 引擎：打字机 / 翻页 / 自动 / 快进 / 选项 / 好感变量 / 结局
│   └── assets/              # 内置默认 UI 资产
├── docs/                    # 其他文档
└── tests/                   # 自动化测试
```
...
| 阶段 5 | 一键生成 `.exe` | ✅ 已完成 |
| 阶段 6 | AI 配置页、语音生成、游戏内置自由对话系统 | ⏳ 待开发 |
| 阶段 7 | 一键生成 `.apk` | ⏳ 待开发 |
