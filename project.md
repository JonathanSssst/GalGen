...
| v2.3 | 2026-08-29 | 分支树改为**流程图**（方块+箭头、SVG 自动布局、鼠标拖拽平移、选项分支箭头标注选项内容、指向已出现剧情的分支以回绕虚线箭头展示）；游戏端**函数跳转/结局生效**（选项/剧情绑定函数真正执行 jump_to / ending_id，不再被自然推进覆盖）；剧情新增显式 **next_id**（`""`=分支终点 / 具体剧情 ID=直接跳转 / 缺省=按 order 自动推进），分支树与游戏端按 next_id 推进，解决按 order 自动续接导致分支串线问题（剧情表单新增「下一剧情」下拉编辑，分支终点以虚线框「终点」标记） |
